...
class Mimic(object):
  ...
  def mimic_dict(self, filename):
    ...
    for word in words :
      if not key in self.m_dict:
        self.m_dict[key] = [word]
      else :
        self.m_dict[key].append(word)
      key = word
    print self.m_dict
  ...
  def print_mimic(self, word):
    ...
    for i in range(200) :
      print word,
      next_word = self.m_dict.get(word)
      if not next_word :
        next_word = self.m_dict['']
